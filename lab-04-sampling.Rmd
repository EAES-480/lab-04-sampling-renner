...
> GM GRADE 9.75/10 - Good effort with solid writing and thoughtful mechanistic comments in several places (especially the reflection and Part II). However, there are some issue: an empty choose-response prompt, and some interpretive inaccuracies.
...
### Response variable = ecosystem respiration (R_eco)
...
> GM: -0.25 RECO is right-skewed (many low/near-zero values with a tail of higher positive values), not left-skewed.
...
ggplot(tibble(mean_est = means), aes(mean_est)) +
  ...
  labs(
    ...
    y = "Count",
    title = "Sampling distribution of the mean (SRS), n = 500",
...
ggplot(tibble(mean_est = means), aes(mean_est)) +
  ...
  labs(
    x = paste0("Sample mean of ", response_var),
    y = "Count",
    title = "Sampling distribution of the mean (SRS), n =1000",
    subtitle = "Dashed line = population mean"
  )
```
...
> *The sampling distribution is almost perfectly centered on the population mean. If I increase n_samp, the distribution becomes more perfectly centered on the population mean.*
...
```{r strat_by_month, echo=FALSE, eval=TRUE}
set.seed(480)
...
> *Both strategies approximate the mean and SD about the same. Stratification might not help because there are not very clear strata. Stratified sampling would make more sense for a different population; for example, sampling R_eco across multiple types of prairies (wet, mesic, tallgrass, etc.)*
...
> *Seasonality changes all of the metrics here significantly - we saw that from the visualizations during the in-class activity. If we ignore the latent grouping variables, we introduce a source of bias and the resulting statistics become less accurate. Stratification is essential for field studies with clear groupings of metrics that do not overlap - for example, day and night, wet/dry prairie, etc. It's best when there are groups (especially small ones). One trade off is that it is more expensive and requires more input. *
...
> *Systematic sampling had a better approximation of sd and SRS had a better approximation of mean. Daily cycles might be interacting with k because I chose 24 hours (daily). *
...
> *Days that are closer together are more similar - this is called temporal autocorrelation. This would increase variance.*
...
df %>%
  filter(month == "Apr") %>%   # e.g., "Jul" if month is labeled
...
> *Yes - both the seasonal and daily patterns are preserved. Quasi-continuous samples will be preferred in situations where response variables are changing quickly. It might be fine to measure plant cover monthly because it won't change that much, but measuring pollinator activity (which changes rapidly) monthly will miss a huge amount of variation.*
...
> *The quasi-continuous is best. This makes sense - quasi-continuous can account best for seasonal/diurnal structure because it monitors nearly continuously. It also doesn't create temporal autocorrelation like clustering. If I had to choose a different method, I would choose SRS and systematic since they have the most similar mean and sd. *


> GM: Good reasoning — quasi-continuous performed best in your run because it captures both seasonal and diurnal structure effectively.
